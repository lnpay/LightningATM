...
logger = logging.getLogger("LNPAY")


def print_conf():
    print(config.conf["lnpay"]["creds"])
...
def request_lnurl(amt):
    ...
    """
    data = {
        "num_satoshis": str(math.floor(amt)),
        "memo": "LightningATM"
    }
    response = requests.post(
        config.conf["lnpay"]["endpoint"] + '/wallet/' + config.conf["lnpay"]["wallet_access_key"] + '/lnurl/withdraw',
        headers={"X-Api-Key": config.conf["lnpay"]["api_key"]},
...
def get_lnurl_balance():
    ...
    response = requests.get(
        config.conf["lnpay"]["endpoint"] + '/wallet/' + config.conf["lnpay"]["wallet_access_key"],
        headers={"X-Api-Key": config.conf["lnpay"]["api_key"]},
    )
